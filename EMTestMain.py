...
def scoreParentSet(childNodeIndex, candidateParentSet, data, startPoint):
    ...
    # 3-5 Bunny Kitten Code Change
    # This goes out
    # gradientDescentKL = GradientDescent(objectiveKL, gradientsKL)
    # gresultKL = gradientDescentKL.descent(startPoint)
    # This goes in
    #print(data.data)    
    emAlgo = EM(data, childNodeIndex, candidateParentSet)
    #startPoint = [0.01] * len(startPoint)

    bad, gresultKL = emAlgo.compute(startPoint)
    if bad:
        return 1000000, gresultKL
...
f=open("ERR_EM_"+ str(len(childParentSet)+1) + "_" + str(d.numberOfRows), "a+")
